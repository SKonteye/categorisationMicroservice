...
SELECT
    ...
    (SELECT COALESCE(COUNT(*), 0) FROM element WHERE element.categorie_id = RecursiveTable.id) AS nbre_element,
    categorie.nom
FROM
    RecursiveTable
        JOIN
...
DROP FUNCTION IF EXISTS public."getNbreelementForCategorieId"(IN categorie_id bigint);

CREATE OR REPLACE FUNCTION public."getNbreelementForCategorieId"(IN categorie_id bigint)
    RETURNS numeric AS
    $$
        DECLARE retour numeric = 0;
        BEGIN
            IF $1 IS NOT NULL THEN
                SELECT COALESCE(COUNT(*), 0) FROM element WHERE element.categorie_id = $1 into retour;
            END IF;
            RETURN retour;
        END
    $$
...
ALTER FUNCTION public."getNbreelementForCategorieId"(IN categorie_id bigint)
    OWNER TO postgres;
